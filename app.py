...
# --- 2. THE 110+ UNIQUE DATABASE (NO LOOPS) ---
def init_db():
    ...
    r_list = [
        ...
        ('Shahi Gatte', 'besan,mawa,curd,spices', 'Rich version of gatte in royal gravy.', 'Rajasthani'),
        ...
        ('Avial', 'mixed veggies,coconut,curd,coconut oil', 'Traditional Kerala vegetable stew.', 'South Indian'),
        ...
        ('Panna Cotta', 'cream,sugar,gelatin,vanilla', 'Italian molded chilled cream dessert.', 'Italian'),
        ('Ravioli', 'flour,egg,spinach,ricotta', 'Stuffed pasta pillows in sauce.', 'Italian'),
        ...
        ('Matar Paneer', 'paneer,peas,tomato,garlic', 'Classic peas and paneer gravy.', 'Punjabi'),
        ('Amritsari Kulcha', 'maida,potato,butter,spices', 'Stuffed tandoori bread from Amritsar.', 'Punjabi'),
        ...
        ('Ghughra', 'maida,mawa,sugar,dry fruits', 'Gujarati style sweet dumplings.', 'Gujarati'),
        ('Sev Tameta', 'tomato,sev,onion,spices', 'Tangy tomato curry topped with sev.', 'Gujarati'),

        # GLOBAL & SNACKS (91-110)
...
# --- PRECISION MATCHER ---
    ...
    p_set = {r[0].lower().strip() for r in db_conn.cursor().execute("SELECT item FROM pantry").fetchall()}
    ...
    for r in recipes:
        ...
        if ing_list:
            ...
            if pct > 0:
                with st.container():
                    st.markdown(f'<div class="match-card"><h3>{name} ({cuis}) - {pct}% Match</h3>', unsafe_allow_html=True)
                    st.progress(pct/100)
                    ...
                    st.markdown(f'<div class="recipe-box"><b>📖 Instructions:</b> {instr}</div></div>', unsafe_allow_html=True)
    ...
    for m in sorted(list(missing))[:30]:
        with st.container():
            ...
            if c_check.checkbox(f"Buy {m.title()}", key=f"buy_{m}"):
                selected.append((m, c_exp.date_input("Set Exp", key=f"exp_{m}"), c_qty.text_input("Set Qty", "1 unit", key=f"qty_{m}")))
    
    if st.button("🛒 Confirm Purchase"):
        for item, ex, q in selected:
...
# --- DIET PLANNER ---
    ...
    st.info(f"BMI: {bmi} | Daily Calories: {int((10*w)+(6.25*h)-120)} kcal")
